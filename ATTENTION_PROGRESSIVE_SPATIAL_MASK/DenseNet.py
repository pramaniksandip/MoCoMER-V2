...
class DenseNet(nn.Module):
    def __init__(
        ...
        # Only one self-attention module after the last dense block
        #self.attention = SelfAttention(self.out_channels)
    ...
    def forward(self, x, x_mask=None):
        if x_mask is None:
            x_mask = torch.ones((x.size(0), x.size(2), x.size(3)), dtype=torch.float32, device=x.device)
        ...
        # Pass through dense block 2
        out = self.dense2(out)
        out = self.trans2(out)
        out_mask = out_mask[:, 0::2, 0::2]

        # Pass through dense block 3
        out = self.dense3(out)
